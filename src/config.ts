...
export function loadConfig(): AppConfig {
  ...
  const extraHosts = readCommaList("EXTRA_ALLOWED_HOSTS").map(normalizeAllowedHost);
  const allowedHosts = Array.from(
    new Set(["127.0.0.1", "localhost", "[::1]", normalizeAllowedHost(host), publicHost, ...extraHosts]),
  );
